...
本研究利用 \texttt{iperf3} 工具產生 UDP 流量，模擬異質業務負載。該工具可於測試端輸出吞吐量、抖動與封包遺失等統計資訊，適合用於本研究之切片服務品質量測 \cite{iperf3_manual}。透過在用戶設備端之容器命名空間內進行量測，定義以下三項關鍵指標：
\begin{itemize}
...
\begin{itemize}
    ...
    \item \textbf{狀態同步時延 ($T_{sync}$)}：定義為從 Porch 完成發佈（$T_1$）至工作負載叢集之 ConfigSync 完成 Git 資源拉取（$T_2$）之時間。此階段受限於 GitOps 之輪詢週期與 ConfigSync 同步機制 \cite{google_config_sync,cncf_gitops_2025}。
...
本實驗旨在建立未經雲原生編排框架介入時之網路效能基準，透過在 Ubuntu 實體主機上直接部署 srsRAN 與 free5GC，驗證 5G 協議棧在無線存取網端實施資源隔離的可行性。本實驗不涉及 Kubernetes 資源調度與意圖轉譯流程，僅作為後續自動化編排之效能對照組。

\subsubsection{實驗設計與環境配置}
本實驗註冊兩組用戶設備（UE），其 IMSI 與切片對應關係如\ref{subsec:multi_slice_ue_config}中表 \ref{tab:ue_slice_mapping} 所示。在 QoS 參數設定上，eMBB 切片採用 5QI 9（Non-GBR），而 URLLC 切片則配置為 5QI 82（Delay Critical GBR）。
...
本實驗利用 \texttt{iperf3} 工具模擬真實業務負載，並透過 UDP 模式產生固定速率之競爭流量。為排除外網延遲干擾，測試流量於本機建立之 \texttt{dummy interface} 上運行，並透過網路命名空間（Network Namespace）與 \texttt{bind-dev} 技術強制流量進入 UE 之隧道介面。測試流程定義如下：

\begin{enumerate}
    \item \textbf{階段一 (0--15s)}：啟動 eMBB 流量測試（UE1），設定為 3 Mbps 之 UDP 負載。
    \item \textbf{階段二 (15--45s)}：於第 15 秒加入 URLLC 流量測試（UE2），設定為 3 Mbps 之 UDP 負載，模擬異質切片競爭無線資源之情境。
...
\begin{itemize}
    ...
    \item \textbf{虛擬化損耗評估}：受限於 VM 與 Kubernetes 網路層（Multus/CNI）之封裝開銷，本實驗之最大可用吞吐量設定為 1.5 Mbps，較實體機之 3 Mbps 為低。由於 Multus 會為 Pod 掛載額外網路介面以承載不同邏輯鏈路，此多介面封裝雖能滿足電信網元之隔離需求，但也會增加容器網路處理路徑之複雜度 \cite{multus_cni}。這證實了在雲原生環境下，雖然資源隔離邏輯（PRB/Priority）能正確實施，但底層虛擬化與 CISM 平台之介入確實會引入額外之處理成本。
...
\begin{itemize}
    ...
    \item \textbf{GitOps 狀態同步 ($T_2 - T_1$)}：平均耗時約 22.66 秒。此為全流程之主要時延來源，係因 ConfigSync 需從 Git 倉庫拉取並套用宣告式資源，其同步週期與網路開銷會直接影響端到端編排時延 \cite{google_config_sync}。
...
\begin{itemize}
    ...
    \item \textbf{Observed Generation}：驗證編排器所觀察到之世代（Generation）與意圖模型之元數據（Metadata）一致，確保無配置漂移或處理遺漏。
...
本章透過三項關鍵實驗完整驗證了本研究所提之架構。實驗一確立了實體環境下之資源隔離基準；實驗二證明了 Nephio 雲原生編排能達成與實體環境一致之隔離效能；實驗三則量化了編排時延，並透過閉環機制確認了配置之最終一致性。實驗結果顯示，本系統能有效將抽象業務期望轉化為實體網路保障，同時大幅降低了跨網域切片之管理門檻。
