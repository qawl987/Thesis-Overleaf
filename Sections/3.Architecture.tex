...
此資源是落實存取網切片意圖的核心關鍵，詳細定義了分散單元（DU）的實體參數以及無線資源排程策略。在物理層配置方面，其包含了下行絕對頻點識別碼（\texttt{dlArfcn}）、運作頻段（\texttt{band}）、通道頻寬與子載波間隔（\texttt{commonScs}）等參數。更重要的是，該資源承載了來自端到端編排器轉譯後的意圖參數，透過定義各組 S-NSSAI 專屬之最小與最大實體資源區塊比例（PRB Ratio）以及調度優先級（\texttt{Priority}），在底層排程器中實現無線資源的硬性隔離與差別化服務品質。

\subsubsection{PLMNConfig：網路識別層 (Network Identity Layer)}
\texttt{PLMNConfig} 資源定義了行動網路的識別邏輯，確保用戶設備（UE）能正確識別並接取目標切片。該模型維護了特定追蹤區內支援的切片清單（S-NSSAI List），並由編排器執行自動化同步，確保此處的 SST/SD 資訊與 \texttt{SrsRANCellConfig} 中的排程器配置嚴格一致。
...
在本研究的驗證體系中，用戶設備（UE）端點必須能精確模擬異質業務的接入行為。透過 \texttt{srsran-helm} 專案，本系統實作了具備切片感知能力的多用戶配置架構，確保每個 UE 實例能與核心網及存取網中預定義的特定切片正確對接。表 \ref{tab:ue_slice_mapping} 詳細展示了用戶設備參數、網路切片識別碼與虛擬射頻端點之間的完整對應關係。
