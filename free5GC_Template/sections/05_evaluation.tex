...
In the Kubernetes testbed, the maximum stable offered rate is lower than in the local baseline. This result is not surprising because the experiment runs the 5G core, RAN components, UE emulation, and automation services inside a virtualized cluster.
...
Figure~\ref{fig:latency} and Table~\ref{tab:latency} show that total orchestration latency is about 30 seconds in the representative run. GitOps synchronization dominates the delay, while the OCUDU Operator completes reference resolution, validation, and rendering in 0.068 seconds. The design is therefore suitable for slice provisioning and policy updates, but fast per-packet or per-TTI adaptation should remain inside the RAN scheduler or a near-real-time control loop.

\subsection{Summary of Findings}

The evaluation supports four findings. The CRD-based intent interface expresses multiple slice objectives and preserves translated parameters; the domain operators reconcile synchronized KRM resources into RAN and core deployments; the generated configuration changes traffic allocation in the expected direction; and the orchestration path is dominated by GitOps synchronization rather than domain reconciliation.
