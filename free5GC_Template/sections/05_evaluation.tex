...
The first experiment validates the generated slice configuration in a local environment. Two UEs are associated with different slices. UE1 is mapped to eMBB with 5QI 6 and a partial PRB share. UE2 is mapped to URLLC with a delay-critical 5QI and a full PRB share. The traffic pattern has three phases: only eMBB is active from 0--15 seconds, both eMBB and URLLC are active from 15--45 seconds, and only eMBB remains active from 45--60 seconds.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{iperf_new.png}
...
The second experiment runs the orchestration path through the Kubernetes and Nephio environment. The same eMBB and URLLC intent is submitted as a CRD. The orchestrator mutates the RAN package, Config Sync applies it to the workload cluster, and the OCUDU Operator validates the synchronized resources and renders the final configuration. In parallel, the free5GC Operator maintains the core NF deployment and the core-facing path applies subscriber QoS state.

\begin{figure}[t]
  \centering
  \includegraphics[width=\linewidth]{iperf_new_nephio.png}
...
The cloud-native experiment also validates configuration consistency across asynchronous control loops. The E2E orchestrator completes its mutation before Config Sync applies the package; the domain operators then reconcile resources after they reach the workload cluster. A correct final result therefore requires these loops to converge on the same desired state. The observed traffic behavior provides data-plane evidence of this convergence, while the fulfillment checks in the next subsection provide management-plane evidence.
...
These checks show that the declarative request, package artifact, and runtime configuration are aligned. They also localize faults: if the package is correct but the ConfigMap is not, the likely issue is in OCUDU reconciliation rather than in Nephio delivery or intent translation.
...
Figure~\ref{fig:latency} and Table~\ref{tab:latency} show that total orchestration latency is about 30 seconds in the representative run. The dominant component is GitOps synchronization, not OCUDU reconciliation. This is an important engineering trade-off: GitOps adds auditability and rollback, but it is not a sub-second control loop. The design is therefore suitable for slice provisioning and policy updates, while fast per-packet or per-TTI adaptation should remain inside the RAN scheduler or a near-real-time control loop.

The result also gives guidance for future optimization. The orchestrator and Porch stage contains translation, package mutation, and approval. The OCUDU Operator completes reference resolution, validation, and rendering in 0.068 seconds in this sample, showing that domain realization is not the bottleneck. Reducing total actuation time would mainly require changing the synchronization policy, shortening the Config Sync interval, or adding a controlled fast path.

\subsection{Summary of Findings}

The evaluation supports four findings. First, the CRD-based intent interface expresses multiple slice objectives and preserves their translated parameters. Second, the domain operators reconcile synchronized KRM resources into RAN and core deployments. Third, the generated configuration changes traffic allocation in the expected direction. Fourth, the orchestration path is dominated by GitOps synchronization rather than domain reconciliation. The design is appropriate for provisioning and policy-level reconfiguration, but not for millisecond-scale radio adaptation.
