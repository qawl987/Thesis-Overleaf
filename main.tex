...
% 論文名稱
\newcommand{\chineseTitle}{意圖驅動之5G無線接取網與核心網切片編排系統}
\newcommand{\englishTitle}{Intent-driven Orchestration System of 5G RAN and Core Network Slices}
% 論文名稱太長的話, 可以修改covers/inside_var.tex 的顯示語法, 詳情寫在那邊

% 自己的名字 (英文名字有兩種寫法, 一個是姓在後, 一個是放前面還有逗號. )
\newcommand{\studentCnName}{丁方凱}
\newcommand{\studentEnName}{Fang Kai Ting} % 書名頁
\newcommand{\studentEnNameA}{Ting, Fang Kai} % 封面用

% 教授的名字
\newcommand{\advisorCnName}{陳志成}
\newcommand{\advisorEnName}{Jyh-Cheng Chen}     % 書名頁
\newcommand{\advisorEnNamex}{Chen, Jyh-Cheng}   % 封面用
...
\input{Sections/5.Conclusion_Futurework} \newpage
